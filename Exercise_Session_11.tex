...
\begin{enumerate}
  ...
  \item Given we know that $s$ is constant,
    ...
    $$
                cos\theta = \frac{x}{s}
    \Rightarrow -sin\theta\theta' = \frac{x'}{s}
    \Rightarrow \theta' = \frac{x'}{s(-sin\theta)}
    $$

    Then we substitute with \textcircled{2}:
    $$
    \theta' = \frac{x'}{s (-\frac{y}{s})} = \frac{5}{13 (-\frac{5}{13})} = -\frac{5}{5}
    \Rightarrow \theanswer{$\theta' = -1 rad/sec$}
    $$
\end{enumerate}
