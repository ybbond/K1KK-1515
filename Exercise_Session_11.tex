...
\newcommand{\Difft}[1]{$d#1/dt$}
\newcommand{\difft}[1]{$\frac{d#1}{dt}$}
...
\newline The source for this document can be found on \href{https://github.com/ybbond/K1KK-1515}{github:ybbond/K1KK-1515}.
...
\begin{wrapfigure}{l}{0.22\textwidth}
  \includegraphics[width=0.2\textwidth]{./assets/e11_1.png}
\end{wrapfigure}
...
\begin{enumerate}
  \item What is the value of \Difft{V}?
  \item What is the value of \Difft{I}?
  \item What equation relates \Difft{R} to \Difft{V} and \Difft{I}?
...
\noindent I will write differentiation with the shorthand. So for example \Difft{V} will be $V'$.
...
\noindent In this exercise, we implement differentiation for known equation \textbf{Ohm's Law}.
...
\begin{enumerate}
  \item Given the equation for \textbf{Pythagoras Theorem} $\fbox{$s^2 = x^2 + y^2$}$.
...
\begin{enumerate}
  \item We will use \textbf{Pythagoras Theorem} $\fbox{$s^2 = x^2 + y^2$}$.
  ...
  \item Given we know that $y$ is constant,
    ...
    % To start, we use derivative from \textcircled{1}:
    % $$
    % cos\theta = \frac{y}{s} \Rightarrow cos\theta = \frac{6}{s}
    % \Rightarrow -sin\theta\theta' = \frac{y}{s'}
    % \Rightarrow \theta' = \frac{y}{s'(-sin\theta)}
    % $$
    % $$
    % \theta' = \frac{y}{s'(-\frac{x}{s})} = \frac{6}{2(-\frac{8}{10})} = -\frac{6}{1,6}
    % \Rightarrow \theanswer{$\theta' = 3,75 rad/sec$}
    % $$
    %
    % Then with \textcircled{2} $\fbox{$sin\theta = \frac{x}{s}$}$, we can substitute:
\end{enumerate}
...
\noindent Given $x$ and $y$ is known, we need to find the diagonal distance $s$ using \textbf{Pythagoras Theorem}.
...
\newline We use the differentiation of \textbf{Pythagoras Theorem} with respect of time.
